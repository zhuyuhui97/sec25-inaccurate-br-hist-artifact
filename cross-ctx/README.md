...
Then, to run both `scope-reader` and `scope-writer` simultaneously on the same CPU core, please use the helper script `scope-userspace.sh`, where the first argument (e.g. 5) specifies the core to pin with `taskset`:

```bash
bash ./scope-userspace.sh 5
```
...
For example, in some environments, when running `taskset -c <CORE_ID> ./scope-reader` **built with the `DBG_RECV_FROM_USERSPACE` flag**, without `./scope-writer` running, you may observe that not all received bits are zeroed:
...
This is a noise generated by other programs sharing the system, and it also jams the results received by the `./scope-reader`. In this case, while you should see `11010110`, you might finally get the following results:
...
This suggests that the side channel for the last bit is fully jammed, consistent with the observations reported in our paper.
